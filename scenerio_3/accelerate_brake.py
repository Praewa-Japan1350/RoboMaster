...
def main():
    ...
    print("Start experiment: stationary 3s -> accelerate -> run 3s -> brake")
    time.sleep(3)

    # 1) Start from rest and stay still for 3 seconds
    # 2) Accelerate (short forward move to ramp speed)
    try:
        ep_chassis.move(x=0.3, y=0, z=0, xy_speed=0.2).wait_for_completed()
    except Exception:
        pass

    # 3) Run straight for 3 seconds at approx constant speed
    try:
        # Prefer drive_speed if available (sets speed directly)
        ep_chassis.drive_speed(x=0.45, y=0, z=0)
        time.sleep(3)
        # Immediate brake
        ep_chassis.drive_speed(x=0, y=0, z=0)
    except Exception:
        # Fallback: start a move and attempt to stop after 3s
        try:
            task = ep_chassis.move(x=3.0, y=0, z=0, xy_speed=0.45)
        except Exception:
            task = None
        time.sleep(3)
        try:
    ...
    print("Experiment finished; saving and closing...")
    ...
    print("Saved: ACC.csv, GYRO.csv, Distance.csv in scenerio_3")
